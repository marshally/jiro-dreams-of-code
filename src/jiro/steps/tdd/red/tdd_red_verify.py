"""Verification for TDD red step.

Verifies that the TDD red step result meets the verification rules:
...
class TddRedVerify(Verification):
    ...
    def _verify_test_fails(self, result: Result) -> str:
        ...
        Runs pytest with --co (collect-only) and --no-cov to get test output.
        ...
        # Use --co to just check if test would run
        try:
            ...
            # If test passed (exit code 0), that's a problem
    ...
    def _verify_test_is_skipped(self, result: Result) -> None:
        ...
        pattern = (
            r"@pytest\.mark\.skip\s*\([^)]*reason\s*=\s*[\"']TDD red:[^\"']*[\"'][^)]*\)\s*\n"
        ...
        if not re.search(pattern, content):
            raise VerificationError(
                ...
                expected=f'Test {test_name} to have @pytest.mark.skip(reason="TDD red: ...")',
