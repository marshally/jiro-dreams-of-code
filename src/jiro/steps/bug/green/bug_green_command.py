...
class BugGreenCommand(Command):
    ...
    def __init__(self, client: AgentClient | None = None) -> None:
        """Initialize BugGreenCommand.

        Args:
            client: AgentClient for executing the bug green agent.
        """
        self.client = client

    async def execute(self, *, step: PlanStep, task: Task) -> BugGreenResult:
        """Execute the bug green command.

        Spawns a subagent to implement code that fixes the bug based on planning context.
        ...
        if self.client is None:
            raise ValueError("AgentClient not configured for BugGreenCommand")

        start_time = time.monotonic()

        # Build prompt for bug green agent
        prompt = self._build_bug_green_prompt(step, task)

        # Execute the agent
        result = await self.client.execute(prompt)

        if not result.success:
            raise ValueError(f"Bug green agent execution failed: {result.error}")
    ...
    def _build_bug_green_prompt(self, step: PlanStep, task: Task) -> str:
        """Build the prompt for the bug green agent.
        ...
        Returns:
            Formatted prompt for the bug green agent.
        """
        return f"""You are a bug fix agent. Your task is to implement code that fixes a bug and makes the failing test pass.
...
Execute the bug green step now."""
