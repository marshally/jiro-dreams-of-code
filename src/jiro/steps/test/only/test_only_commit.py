...
class TestOnlyCommit(Commit):
    """Commit class for test only step.
    ...
    def create(
        ...
        *,
        ...
        """Create a git commit for the test only step.
        ...
        test_specifier = getattr(result, "test_specifier", "unknown")
        # Extract just the test name from the specifier
        test_name = test_specifier.split("::")[-1] if "::" in test_specifier else "test"
        message = f"🧪 Add test for {test_name}"
